 ...
 */
	function loginUser(){
		if (isset($_POST['inpEIN']) && isset($_POST['inpPass'])){
			...
			$pass = md5($password);
			$query = "SELECT pupil_cin,parent_pwd FROM parent_table WHERE pupil_cin='$usrCIN' AND parent_pwd='$pass' ";
			$result = mysqli_query($_CON,$query) or die(mysqli_connect_error());
			$count = mysqli_num_rows($result);
			if($count == 1){
				$_SESSION['parent_usr'] = $usrCIN;
			}else{
		...
		$usrCIN = $_SESSION['parent_usr'];
		setcookie("parent_usr", $usrCIN, strtotime( '+30 days' ), "/", "", "", TRUE);
