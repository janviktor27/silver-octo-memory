 ...
 */
	...
	function loginUser(){
		if (isset($_POST['inpEIN']) && isset($_POST['inpPass'])){
			...
			$pass = md5($password);
			$query = "SELECT * FROM teachers_table WHERE teacher_ein='$usrCIN' AND teacher_pwd='$pass' AND user_type='2' ";
			$result = mysqli_query($_CON,$query) or die(mysqli_connect_error());
			$count = mysqli_num_rows($result);
			if($count == 1){
				$_SESSION['teacher_usr'] = $usrCIN;
			}else{
		...
		$usrCIN = $_SESSION['teacher_usr'];
		$password = $_SESSION['password'];
		setcookie("teacher_usr", $usrCIN, strtotime( '+30 days' ), "/", "", "", TRUE);
